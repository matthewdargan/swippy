...
type FindingClient struct {
	*http.Client        // make requests to the eBay Finding API
	AppID        string // eBay application ID
	URL          string // specifies the eBay Finding API endpoint
}
...
func (c *FindingClient) FindItemsAdvanced(params map[string]string) (*FindItemsAdvancedResponse, error) {
	req, err := c.newRequest(operationAdvanced, params)
	if err != nil {
...
//
// [Searching and Browsing By Category]: https://developer.ebay.com/api-docs/user-guides/static/finding-user-guide/finding-searching-browsing-by-category.html
func (c *FindingClient) FindItemsByCategory(params map[string]string) (*FindItemsByCategoryResponse, error) {
	req, err := c.newRequest(operationCategory, params)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrNewRequest, err)
	}
	resp, err := c.Do(req)
...
func (c *FindingClient) FindItemsByKeywords(params map[string]string) (*FindItemsByKeywordsResponse, error) {
	req, err := c.newRequest(operationKeywords, params)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrNewRequest, err)
	}
	resp, err := c.Do(req)
...
func (c *FindingClient) FindItemsByProduct(params map[string]string) (*FindItemsByProductResponse, error) {
	req, err := c.newRequest(operationProduct, params)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrNewRequest, err)
	}
	resp, err := c.Do(req)
...
func (c *FindingClient) FindItemsInEBayStores(params map[string]string) (*FindItemsInEBayStoresResponse, error) {
	req, err := c.newRequest(operationStores, params)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", ErrNewRequest, err)
	}
	resp, err := c.Do(req)
...
func (c *FindingClient) newRequest(op string, params map[string]string) (*http.Request, error) {
